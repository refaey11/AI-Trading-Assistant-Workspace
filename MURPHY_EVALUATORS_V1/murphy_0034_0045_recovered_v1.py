...
def wave2(wave1_high: Optional[float], wave1_low: Optional[float],
          wave2_extreme: Optional[float]) -> Result:
...
def wave3(length1: Optional[float], length3: Optional[float],
          length5: Optional[float]) -> Result:
...
def wave4(wave1_low: Optional[float], wave1_high: Optional[float],
          wave4_price: Optional[float]) -> Result:
...
def system_discipline(system_defined: Optional[bool], regime_checked: Optional[bool]) -> Result:
    ...
    return Result("PASS","Systematic process and regime evaluation are present.") if system_defined and regime_checked else Result("FAIL","Required discipline/regime gate is absent.")

def psar_regime(trending: Optional[bool]) -> Result:
    if trending is None:
        return Result("NOT_EVALUABLE","Missing market-regime classification.")
    return Result("PASS","Trending regime is compatible with Parabolic SAR.") if trending else Result("FAIL","Sideways/non-trending regime is not suitable for this trend-following use.")
...
def adx_regime(adx: Optional[float], threshold: Optional[float]) -> Result:
    ...
    return Result("PASS","ADX is above the supplied trend-regime threshold.") if adx >= threshold else Result("FAIL","ADX is below the supplied trend-regime threshold.")
...
def capital_reserve(invested_pct: Optional[float]) -> Result:
    ...
    return Result("PASS","Investment is within the 50% guideline.") if invested_pct <= 50 else Result("FAIL","Investment exceeds the 50% guideline.")
...
def single_market_exposure(exposure_pct: Optional[float]) -> Result:
    ...
    return Result("PASS","Exposure is within the 10–15% guideline.") if exposure_pct <= 15 else Result("FAIL","Exposure exceeds the 15% ceiling.")

def market_risk(risk_pct: Optional[float]) -> Result:
    if risk_pct is None:
        return Result("NOT_EVALUABLE","Missing single-market risk.")
    return Result("PASS","Risk is within the 5% guideline.") if risk_pct <= 5 else Result("FAIL","Risk exceeds the 5% guideline.")

def total_margin(margin_pct: Optional[float]) -> Result:
    if margin_pct is None:
        return Result("NOT_EVALUABLE","Missing total-margin percentage.")
    return Result("PASS","Margin is within the 20–25% guideline.") if margin_pct <= 25 else Result("FAIL","Margin exceeds the 25% ceiling.")
