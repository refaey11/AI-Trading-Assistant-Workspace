...
def main():
 ...
 if a.year>=LOCKED: raise ValueError('2025_OOS_LOCKED')
 state=read(a.market_state,{'timestamp','close'}); mtf=read(a.mtf,{'timestamp','trend','h4_trend','mtf_state'}); h=read(a.h1,{'timestamp','open','high','low','close'}); m=read(a.m1,{'timestamp','open','high','low','close','volume'})
 mtf=mtf.rename(columns={'trend':'H1_trend','h4_trend':'H4_trend','mtf_state':'MTF_state'})
 h['f12']=h.close.shift(-12); h['f24']=h.close.shift(-24); h['f48']=h.close.shift(-48)
 mur=build_murphy(h.drop(columns=['f12','f24','f48']),m).set_index('timestamp'); mtf=mtf.set_index('timestamp'); h=h.set_index('timestamp'); state=state[state['timestamp'].dt.year.eq(a.year)].set_index('timestamp')
 rows=[]
 for ts,_ in state.iterrows():
  if ts not in mtf.index or ts not in h.index or ts not in mur.index: continue
  tr=mtf.loc[ts]; bar=h.loc[ts]; md=str(mur.loc[ts,'murphy_direction'])
  ass=decision_brain.assess({'mtf_trend_score':(score(tr.H4_trend)+score(tr.H1_trend))/2.0,'H1_trend_regime':score(tr.H1_trend),'H4_trend_regime':score(tr.H4_trend),'volume_available':False},similarity=None)
  rec={'timestamp':ts.isoformat(),'murphy_direction':md,'brain_state':ass.market_state,'brain_bias':ass.directional_bias,'brain_confidence':float(ass.confidence),'h4_trend':tr.H4_trend,'h1_trend':tr.H1_trend,'mtf_state':tr.MTF_state,'context_bucket':bucket(ass,md,str(tr.MTF_state))}
  for n in (12,24,48):
   fut=bar.get(f'f{n}'); valid=pd.notna(fut) and (ts+pd.Timedelta(hours=n)).year<LOCKED; srtn=None
   if valid and md in DIR:
    raw=float(fut)/float(bar.close)-1.0; srtn=raw if md=='BULLISH' else -raw
 ...
 for name,g in d.groupby('context_bucket',dropna=False):
  z={'context_bucket':name,'signals':int(len(g))}
  for n in (12,24,48):
  ...
  buckets.append(z)
 summary={'status':'PASS_SHADOW_ONLY','mode':'REAL_DATA_PRE2025_CONTEXT_GATE_SHADOW_V4','evaluation_year':a.year,'pair':'GBPUSD','events':int(len(out)),'murphy_directional_events':int(len(d)),'context_buckets':buckets,'brain_role':'context_and_regime_only','murphy_role':'directional_anchor_MURPHY_0021','mtf_schema_mapping':{'trend':'H1','h4_trend':'H4','mtf_state':'combined'},'new_rule_semantics':False,'policy_changed':False,'replacement_pnl':False,'oos_2025_locked':True,'oos_tuning':False,'future_feature_leakage':False}
 a.output.with_suffix('.json').write_text(json.dumps(summary,indent=2,sort_keys=True),encoding='utf-8'); print(json.dumps(summary,indent=2,sort_keys=True))
