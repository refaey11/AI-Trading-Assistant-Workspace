...
def build(*, timestamp: str, h1: Path, market_state: Path, murphy_root: Path, output: Path,
    ...
    frozen_result = frozen.evaluate_frozen_candidate_risk(
        direction=direction, equity=equity, peak_equity=peak_equity,
    ...
    canonical_result = canonical.evaluate_risk(
        canonical.RiskRequest(
            equity=equity, risk_percent=frozen_result.risk_percent,
            entry_price=entry, stop_distance=stop_distance,
            take_profit_distance=reward_distance, stop_mode="structure",
    ...
    fields = {
        "timestamp": ts.isoformat(), "direction": direction,
        "equity": equity, "peak_equity": peak_equity,
        "prior_loss_streak": prior_loss_streak, "entry_price": entry, "atr": atr,
        ...
        "risk_pass": True, "risk_money": equity * frozen_result.risk_percent,
        "rr": 2.0, "risk_budget_locked": True, "stop_mode": "structure",
        "authoritative": True, "authority_scope": "evaluation_single_event",
    ...
    return {
        "status": "PASS", "timestamp": ts.isoformat(), "direction": direction,
        "risk_percent": frozen_result.risk_percent, "stop_loss": frozen_result.stop_loss,
        "take_profit": frozen_result.take_profit, "position_size": frozen_result.position_size,
