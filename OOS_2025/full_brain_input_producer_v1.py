...
def normalize(*, context: Path, murphy: Path, nison: Path, risk: Path, execution: Path, year: int, output_dir: Path) -> dict:
    streams = {name: restrict_year(read_stream(path, REQUIRED[name], name), year) for name, path in {"context": context, "murphy": murphy, "nison": nison, "risk": risk, "execution": execution}.items()}
    ...
    manifest = {"status":"PASS","mode":"FULL_DECISION_BRAIN_INPUT_NORMALIZATION","evaluation_year":year,"common_timestamps":len(common),"streams":{name:{"rows":int(len(df[df["timestamp"].isin(common)])),"source":paths[name]} for name,df in streams.items()},"canonical_policy":{"nison_confirmation_only":True,"tiz_process_only":True,"risk_hard_gate":True,"historical_memory_evidence_only":True,"2025_is_oos":True,"no_tuning":True},"source_backed_only":True,"invented_evidence":False,"ready_for_full_brain_assembler":True}
    (output_dir / "FULL_BRAIN_INPUT_MANIFEST.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
    return manifest

def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--context",required=True,type=Path); p.add_argument("--murphy",required=True,type=Path); p.add_argument("--nison",required=True,type=Path); p.add_argument("--risk",required=True,type=Path); p.add_argument("--execution",required=True,type=Path); p.add_argument("--year",required=True,type=int); p.add_argument("--output-dir",required=True,type=Path)
    a=p.parse_args(); print(json.dumps(normalize(context=a.context,murphy=a.murphy,nison=a.nison,risk=a.risk,execution=a.execution,year=a.year,output_dir=a.output_dir),indent=2)); return 0
