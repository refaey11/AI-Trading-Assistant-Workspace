...
def main():
    ...
    mur["direction"] = first_nonblank(mur["direction"] if "direction" in mur.columns else None,
                                        mur["directional_confirmation"] if "directional_confirmation" in mur.columns else None,
                                        mur.index).str.upper().str.strip()
    mur["source_rule_id"] = first_nonblank(mur["source_rule_id"] if "source_rule_id" in mur.columns else None,
                                            mur["rule_id"] if "rule_id" in mur.columns else None,
                                            mur.index).fillna("")
    ...
    if required_ohlc - set(h1.columns): raise SystemExit("FAIL_CLOSED_H1_MISSING_OHLC")
    for c in required_ohlc: h1[c] = pd.to_numeric(h1[c], errors="coerce")
    ...
    x["volatility_state"] = x.volatility_state.astype("string").str.upper().str.strip()
    x["mtf_context"] = x.mtf_context.astype("string").str.lower().str.strip()

    # Missing evidence is candidate-specific NOT_EVALUABLE, not a reason to
    # discard every candidate. C2/C3 only require volatility + H4/direction;
    # C1 additionally requires an as-of MTF context value.
    ...
    for r in base.itertuples(index=False):
        if pd.isna(r.H4_trend_regime) and (r.direction == "SELL" or r.mtf_context == "strong_bearish"):
            continue
        ...
        for k in range(j,len(h1)):
            b=h1.iloc[k]; hi=float(b.high); lo=float(b.low)
        ...
        rows.append({"event_time":r.timestamp,"direction":r.direction,"source_rule_id":getattr(r,"source_rule_id",""),
                     "volatility_state":r.volatility_state,"mtf_context":r.mtf_context,"H4_trend_regime":r.H4_trend_regime,
                     "entry_time":h1.iloc[j].timestamp,"entry_price":entry,"ATR20_asof":atr,"exit_time":exit_time,"outcome":outcome,"net_R":net})
    ...
    for cid,fn in CANDIDATES.items():
        g=ev[fn(ev)]
        r=pd.to_numeric(g.net_R,errors="coerce").dropna(); wins=r[r>0].sum(); losses=-r[r<0].sum()
        stats.append({"candidate_id":cid,"events":len(g),"evaluated":len(r),"TP":int((g.outcome=="TP").sum()),"SL":int((g.outcome=="SL").sum()),
                      "ambiguous":int((g.outcome=="AMBIGUOUS").sum()),"open":int((g.outcome=="OPEN").sum()),
                      "win_rate":float((r>0).mean()) if len(r) else None,"expectancy_R":float(r.mean()) if len(r) else None,
                      "profit_factor":float(wins/losses) if losses else None,"total_net_R":float(r.sum()) if len(r) else 0.0})
    pd.DataFrame(stats).to_csv(out/"oos_2025_candidate_results.csv",index=False)
    manifest={"status":"OOS_2025_EVALUATION_ONLY","window":"2025-only","candidate_set_frozen":True,"tuning_applied":False,
              "threshold_sweep":False,"future_data_used":False,"strict_asof_context":True,
              "missing_evidence_policy":"candidate-specific NOT_EVALUABLE","atr20_source":"H1_completed_bars_fallback_when_market_state_missing",
              "entry_next_h1_open":True,"stop_atr":0.75,"target_R":2.0,"official_profitability_claim_allowed":False,"canonical_three_book_mode":False}
