...
def main():
 ...
 mur=mur[(mur.status=='PASS')&mur.direction.isin(['BUY','SELL','BULLISH','BEARISH'])].copy(); mur['direction']=mur.direction.replace({'BULLISH':'BUY','BEARISH':'SELL'})
 h1=h1.sort_values('timestamp').reset_index(drop=True)
 for c in ['open','high','low']: h1[c]=pd.to_numeric(h1[c],errors='coerce')
 if {'open','high','low'}-set(h1.columns): raise SystemExit('FAIL_CLOSED_H1_MISSING_OHLC')
 # Frozen ATR20 fallback: derive from H1 only, using completed bars strictly before each event.
 prev_close=h1['close'].shift(1) if 'close' in h1.columns else None
 if prev_close is None: raise SystemExit('FAIL_CLOSED_H1_MISSING_CLOSE')
 ...
 if mtfneed-set(mtf.columns): raise SystemExit('FAIL_CLOSED_MTF_REQUIRED_FIELDS')
 ...
 x=pd.merge_asof(x,h1[['timestamp','ATR20']].sort_values('timestamp'),on='timestamp',direction='backward',allow_exact_matches=False)
 x['ATR20']=pd.to_numeric(x.ATR20,errors='coerce'); x['H4_trend_regime']=pd.to_numeric(x.H4_trend_regime,errors='coerce')
 x=x.dropna(subset=['ATR20','volatility_state','mtf_context','H4_trend_regime']).copy(); x['volatility_state']=x.volatility_state.astype(str).str.upper(); x['mtf_context']=x.mtf_context.astype(str).str.lower()
 ts=h1.timestamp.astype('int64').to_numpy(); rows=[]
 for r in x.itertuples(index=False):
  ...
  result='OPEN'; exit_time=None; net=None
  for k in range(j,len(h1)):
   b=h1.iloc[k]; hi,lo=float(b.high),float(b.low); hit_sl=(lo<=sl) if r.direction=='BUY' else (hi>=sl); hit_tp=(hi>=tp) if r.direction=='BUY' else (lo<=tp)
   if hit_sl and hit_tp: result='AMBIGUOUS'; exit_time=b.timestamp; break
   if hit_tp: result='TP'; exit_time=b.timestamp; net=2.0; break
   if hit_sl: result='SL'; exit_time=b.timestamp; net=-1.0; break
  rows.append({'event_time':r.timestamp,'direction':r.direction,'source_rule_id':getattr(r,'source_rule_id',''),'volatility_state':r.volatility_state,'mtf_context':r.mtf_context,'H4_trend_regime':r.H4_trend_regime,'entry_time':h1.iloc[j].timestamp,'entry_price':entry,'ATR20_asof':atr,'exit_time':exit_time,'outcome':result,'net_R':net})
 ...
 manifest={'status':'OOS_2025_EVALUATION_ONLY','window':'2025-only','candidate_set_frozen':True,'tuning_applied':False,'threshold_sweep':False,'future_data_used':False,'strict_asof_context':True,'atr20_source':'H1_completed_bars_fallback_when_market_state_missing','entry_next_h1_open':True,'stop_atr':0.75,'target_R':2.0,'official_profitability_claim_allowed':False,'canonical_three_book_mode':False}
