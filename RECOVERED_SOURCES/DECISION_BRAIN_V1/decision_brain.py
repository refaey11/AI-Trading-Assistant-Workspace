...
def assess(row: Dict[str, Any], similarity: Dict[str, Any] | None = None) -> MarketAssessment:
    ...
    It produces a market-state assessment only.
    """
    evidence=[]
    ...
    # Trend-regime evidence across timeframes
    regs=[row.get(k) for k in [
    ...
    # Volume evidence only when actually available.
    ...
    no_trade=[]
    if bias in ("neutral","conflicted"):
        no_trade.append("Evidence is not directionally aligned.")
    if confidence < 0.25:
        no_trade.append("Confidence is below the V1 decision threshold.")
    if not bool(row.get("volume_available", False)):
        no_trade.append("Volume confirmation is unavailable for this historical regime.")
