...
# Pick the Murphy CSV that has the actual canonical schema and source_rule_id.
mur=[]
...
nd=pd.read_csv(nison,usecols=[c for c in ['timestamp','source_rule_id','rule_id'] if c in pd.read_csv(nison,nrows=0).columns],low_memory=False)
rule='source_rule_id' if 'source_rule_id' in nd.columns else 'rule_id'
nd.timestamp=pd.to_datetime(nd.timestamp,utc=True,errors='coerce',format='mixed'); nd[rule]=nd[rule].astype('string').str.strip()
...
for p in sorted((root/'unpacked/mtf').rglob('*.csv')):
    try:
        if need.issubset(set(pd.read_csv(p,nrows=0,low_memory=False).columns)):
            mtf.append(p); break
    except Exception: pass
if not mtf: raise SystemExit('BLOCKED_NO_SIX_TF_MTF_SOURCE')
mtfs=tsset(mtf[0])
...
sim_min=min(vals)
common=sorted(h1s & markets & mtfs & ms & set(ns)); common=[t for t in common if 2016<=t.year<=2024 and t>=sim_min]
if not common: raise SystemExit('BLOCKED_NO_SOURCE_BACKED_EVENT_MEETS_ALL_GATES')
print('DISCOVERED_EVENT_TS',common[0].isoformat().replace('+00:00','Z'))
open('/tmp/gate3c_event_ts','w').write(common[0].isoformat().replace('+00:00','Z'))
