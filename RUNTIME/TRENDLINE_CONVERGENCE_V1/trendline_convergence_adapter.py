...
def evaluate_convergence(upper: Dict[str,Any], lower: Dict[str,Any]) -> Dict[str,Any]:
    ...
    if any(upper.get(k) is None for k in required) or any(lower.get(k) is None for k in required):
        return {'status':'NOT_EVALUABLE','converging':None,'reason':'Missing confirmed two-point trendline evidence.'}
    if upper['line_type']!='HIGH' or lower['line_type']!='LOW':
        return {'status':'NOT_EVALUABLE','converging':None,'reason':'Expected HIGH upper line and LOW lower line.'}
    try:
        ...
        gap_rate=su-sl
        if gap_a <= 0:
            return {'status':'NOT_EVALUABLE','converging':None,'reason':'No valid positive upper-over-lower gap at common availability.'}
        return {'status':'PASS','converging':gap_rate < 0,'gap_at_availability':gap_a,'gap_rate':gap_rate,
                'availability_timestamp':a.isoformat(sep=' '),
                'reason':'Exact gap-rate sign from confirmed geometry; no tolerance or threshold added.'}
    except Exception as e:
        return {'status':'NOT_EVALUABLE','converging':None,'reason':f'Geometry unavailable: {type(e).__name__}.'}
