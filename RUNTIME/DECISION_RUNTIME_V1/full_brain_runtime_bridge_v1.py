...
def run_full_brain_cycle(
    *,
    ...
    risk_authoritative = _is_authoritative(risk_evidence)
    ...
    tiz_state = str(
        tiz.get("process_state")
        or tiz.get("process_gate")
        or tiz.get("status")
        or "NOT_EVALUABLE"
    ).upper()

    # Development/OOS can evaluate without synthetic psychological state.
    # Production execution must not pretend that missing TIZ evidence exists.
    ...
    return assemble_decision_event(
        ...
        tiz_evidence={
            "authoritative": tiz_available,
            "process_state": tiz_state,
            "tiz_verified": tiz_available,
            "unverified": not tiz_available,
            "source": tiz.get("source", "TIZ_RUNTIME_BOUNDARY_RESOLUTION_V2"),
        },
        ...
        mode=mode,
        provenance={
            "bridge": "full_brain_runtime_bridge_v1",
            "tiz_optional_when_unavailable": not tiz_available,
        },
    )
