...
def run(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(murphy_ids) != 34:
        return {"gate3c": "BLOCKED", "stage": "MURPHY", "reason": "MURPHY_34_ENVELOPE_REQUIRED", "observed_count": len(murphy_ids), "observed": murphy_ids}
    if len(nison_ids) != 44:
    ...
    # TIZ stays optional/process-only. Missing TIZ is recorded as unverified.
    ...
    result = bridge.run_full_brain_cycle(
        ...
        murphy_evidence={"status": "PASS", "rows": murphy_rows, "authoritative": True, "evidence_set": {rid: r for rid, r in zip(murphy_ids, murphy_rows)}},
        nison_evidence={"status": "PASS", "rows": nison_rows, "authoritative": True, "confirmation": nison.get("confirmation", "ABSENT"), "contradiction": bool(nison.get("contradiction", False)), "evidence_set": {rid: r for rid, r in zip(nison_ids, nison_rows)}},
        risk_evidence=risk,
        tiz_evidence=tiz,
        historical_evidence={
            "historical_context": event.get("historical_context") or {},
            "historical_outcome": event.get("historical_outcome") or {},
            "similarity": event.get("similarity") or {},
    ...
    output = {
        ...
        "rule_envelope": {"murphy": len(murphy_ids), "nison": len(nison_ids)},
        "memory_retrieval": {
            "historical_context": event.get("historical_context") or {},
            "historical_outcome": event.get("historical_outcome") or {},
            "similarity": event.get("similarity") or {},
        ...
        "governance": {
            ...
            "murphy_primary_context": True,
            "nison_direction_generation": False,
            "tiz_direction_generation": False,
            "memory_direction_generation": False,
...
def main() -> int:
    ...
    result = run(json.loads(args.event.read_text(encoding="utf-8")))
