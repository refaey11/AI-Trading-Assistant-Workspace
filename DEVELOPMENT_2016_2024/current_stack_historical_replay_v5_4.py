...
def evaluate_frozen_candidate_risk(*, direction: str, equity: float, peak_equity: float, entry: float, atr: float, prior_loss_streak: int = 0, risk_budget_pct: Optional[float] = None) -> FrozenRiskResult:
...
source = source.replace(
    ...
    '        nids = {rid for ids in ng.expanded_ids for rid in ids}\n        # Do not require all 44 Nison rules at the same timestamp.\n        # Nison rules are event/pattern evidence; absence/failure is not contradiction.\n        if not nids:\n            continue\n'
)
...
if __name__ == "__main__":
    ...
    for name in ("h1", "market-state", "murphy", "nison", "mtf", "historical-context", "historical-outcome", "similarity-artifact", "retrieval-artifact", "scenario-artifact", "output-dir"):
