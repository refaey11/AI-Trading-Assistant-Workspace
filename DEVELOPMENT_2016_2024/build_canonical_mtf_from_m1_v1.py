...
def parse_timestamp(date_col: pd.Series, time_col: pd.Series) -> pd.Series:
    ...
    # Support ISO, YYYY.MM.DD, DD.MM.YYYY, DD/MM/YYYY and broker exports.
    ...
    with zipfile.ZipFile(path) as z:
        ...
        for name in names:
            with z.open(name) as fh:
                raw = pd.read_csv(fh, sep=None, engine='python', header=None, low_memory=False)
            diagnostics['files'].append(name)
            diagnostics['rows_read'] += len(raw)
            if raw.shape[1] < 6:
                continue
            raw = raw.iloc[:, :6]
            raw.columns = ['date', 'time', 'open', 'high', 'low', 'close']
            if str(raw.iloc[0, 0]).strip().lower() in {'date', '<date>', 'datetime', '<datetime>'}:
                raw = raw.iloc[1:]
...
def main():
    ...
    ap.add_argument('--input', required=True); ap.add_argument('--output', required=True); ap.add_argument('--report', required=True)
    a = ap.parse_args()
    m1, diagnostics = load_m1(Path(a.input))
