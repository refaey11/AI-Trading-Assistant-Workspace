...
def run(
    ...
    murphy_by_ts = {ts:g for ts,g in mdf.groupby("timestamp", sort=False)}
    ...
    for bar_idx, bar in bars.iterrows():
        ...
        murphy_envelope = murphy_by_ts.get(ts)
        if murphy_envelope is None or murphy_envelope.empty:
            continue

        murphy_loaded_ids = sorted({
            rid for ids in murphy_envelope.expanded_ids for rid in ids
        })
        murphy_sent_ids = sorted({
            rid for ids in groups.expanded_ids for rid in ids
        })
        murphy_missing_ids = sorted(set(MURPHY_IDS) - set(murphy_loaded_ids))
        murphy_unknown_ids = sorted(set(murphy_loaded_ids) - set(MURPHY_IDS))
        murphy_rows = murphy_envelope.drop(
            columns=["expanded_ids", "direction_norm"],
            errors="ignore",
        ).to_dict("records")
        ...
        for _, mr in groups.iterrows():
            ...
            brain_result = bridge.run_full_brain_cycle(
                ...
                murphy_evidence={
                    ...
                    "rows": murphy_rows,
                    "authoritative": True,
                    "governed_rule_ids": sorted(MURPHY_IDS),
                    "loaded_rule_ids": murphy_loaded_ids,
                    "sent_rule_ids": murphy_sent_ids,
                    "loaded_rule_count": len(murphy_loaded_ids),
                    "sent_rule_count": len(murphy_sent_ids),
                    "missing_rule_ids": murphy_missing_ids,
                    "unknown_rule_ids": murphy_unknown_ids,
                    "envelope_complete": set(murphy_loaded_ids) == MURPHY_IDS,
                },
                ...
                source_rule_ids=sorted(set(murphy_loaded_ids).union(nids)),
                entry_price=entry,
                atr=atr,
                mode="development",
            )
            ...
            event = {
                ...
                "murphy_rule_count": len(murphy_loaded_ids),
                "murphy_loaded_rule_count": len(murphy_loaded_ids),
                "murphy_sent_rule_count": len(murphy_sent_ids),
                "murphy_missing_rule_ids": murphy_missing_ids,
                "murphy_unknown_rule_ids": murphy_unknown_ids,
                "murphy_envelope_complete": set(murphy_loaded_ids) == MURPHY_IDS,
