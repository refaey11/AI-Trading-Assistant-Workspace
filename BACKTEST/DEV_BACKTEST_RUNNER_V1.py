...
def aggregate_murphy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ts, g in df.groupby("timestamp", sort=True):
        dirs = sorted({d for d in (normalize_direction(x) for x in g.loc[g["status"].astype(str).str.upper().eq("PASS"), "direction"]) if d})
        rule_ids = sorted(set(g.get("source_rule_id", pd.Series(dtype=str)).dropna().astype(str)))
        if len(dirs) == 1:
            direction = dirs[0]
        elif len(dirs) > 1:
            ...
            direction = "ABSENT"
        rows.append({"timestamp": ts, "murphy_status": "PASS" if direction in {"BULLISH", "BEARISH"} else "NOT_EVALUABLE", "murphy_direction": direction, "source_rule_ids": json.dumps(rule_ids)})
...
def aggregate_nison(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for ts, g in df.groupby("timestamp", sort=True):
        passed = {d for d in (normalize_direction(x) for x in g.loc[g["status"].astype(str).str.upper().eq("PASS"), "direction"]) if d}
        failed = {d for d in (normalize_direction(x) for x in g.loc[g["status"].astype(str).str.upper().eq("FAIL"), "direction"]) if d}
        confirmation = sorted(passed)[0] if len(passed) == 1 else ("CONFLICTED" if len(passed) > 1 else "ABSENT")
        rows.append({"timestamp": ts, "nison_confirmation": confirmation, "nison_contradiction": bool(failed), "nison_rule_count": int(g["rule_id"].nunique())})
    return pd.DataFrame(rows)
...
def run(*, h1: Path, murphy: Path, nison: Path, context: Path, output_dir: Path) -> dict[str, Any]:
    ...
    murphy_raw = load_csv(murphy, {"timestamp", "status", "direction"})
    nison_raw = load_csv(nison, {"timestamp", "status", "direction", "rule_id"})
    ctx = load_csv(context, {"timestamp", "entry_price", "atr"})

    if pd.to_datetime(h1.stat().st_mtime, unit="s", utc=True) is None:
        raise RuntimeError("Invalid source metadata")

    allowed = allowed_rule_ids()
    observed_m = set(murphy_raw.get("source_rule_id", pd.Series(dtype=str)).dropna().astype(str))
    ...
    m = aggregate_murphy(murphy_raw)
    n = aggregate_nison(nison_raw)
    merged = ctx.merge(m, on="timestamp", how="left").merge(n, on="timestamp", how="left")
    ...
    for idx, row in merged.iterrows():
        ...
        nison_conf = str(row.get("nison_confirmation") or "ABSENT")
        ...
        source_rule_ids = []
        try:
            source_rule_ids.extend(json.loads(row.get("source_rule_ids") or "[]"))
        except Exception:
            pass
        events.append({"timestamp": ts, "murphy_direction": murphy_dir, "nison_confirmation": nison_conf, "nison_contradiction": contradiction, "brain_bias": bias, "brain_confidence": assessment.confidence, "source_rule_ids": json.dumps(source_rule_ids)})

        if murphy_dir not in {"BULLISH", "BEARISH"}:
            continue
        if bias != murphy_dir or contradiction:
            continue
        if pd.isna(row.get("entry_price")) or pd.isna(row.get("atr")) or float(row.get("atr")) <= 0:
            continue
        direction = "BUY" if murphy_dir == "BULLISH" else "SELL"
        # Map timestamp back to authoritative H1 bar; entry is at event close/context close.
        pos = bars.index[bars["timestamp"].eq(ts)]
        if len(pos) == 0:
            continue
        bar_idx = int(pos[0])
        result = simulate_trade(bars, bar_idx, direction, float(row["entry_price"]), float(row["atr"]))
    ...
    events_df.to_csv(output_dir / "unified_78_events_2016_2024.csv", index=False)
    events_df.to_csv(output_dir / "decision_events_2016_2024.csv", index=False)
    ...
    funnel = {
        ...
        "murphy_directional": int(events_df["murphy_direction"].isin(["BULLISH", "BEARISH"]).sum()) if not events_df.empty else 0,
        "decision_aligned": int(((events_df["murphy_direction"] == events_df["brain_bias"]) & events_df["murphy_direction"].isin(["BULLISH", "BEARISH"])).sum()) if not events_df.empty else 0,
    ...
    metrics = {
        ...
        "reason": "Current frozen transaction-cost/slippage contract is not represented in the available runtime inputs; metrics are diagnostic only." if outcome.empty is False else "No executed trades were produced by the current evidence fan-in.",
    }
...
def main() -> int:
    ...
    a = p.parse_args()
    result = run(h1=a.h1, murphy=a.murphy, nison=a.nison, context=a.context, output_dir=a.output_dir)
