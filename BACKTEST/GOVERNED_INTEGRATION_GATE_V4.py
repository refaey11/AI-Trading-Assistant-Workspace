...
def nison_inventory(path: Path):
    ...
    for ch in pd.read_csv(path, usecols=["timestamp", "rule_id", "status", "direction"], chunksize=200_000, low_memory=False):
        ...
        if ch["timestamp"].isna().any():
            raise ValueError("Invalid Nison timestamp")
        cy = set(ch.timestamp.dt.year.astype(int))
        years |= cy
        if 2025 in cy:
            raise ValueError("Nison source contains 2025")
        ch = ch[(ch.timestamp.dt.year >= 2016) & (ch.timestamp.dt.year <= 2024)]
        if ch.empty:
            continue
        observed |= set(ch.rule_id.dropna().astype(str))
        rows += len(ch)
...
def main() -> int:
    ...
    checks["h1_rows"] = int(len(h1))
    checks["h1_years"] = sorted(h1.timestamp.dt.year.unique().tolist())
    market = load_csv(Path(args.market_state), {"timestamp"})
    mtf = load_csv(find_csv(Path(args.mtf), "GBPUSD_MTF_H4_H1.csv"), {"timestamp"})
    murphy = load_csv(find_csv(Path(args.murphy), "MURPHY_2016_2024_FULL_EVIDENCE.csv"), {"timestamp", "status", "direction"}, None)
    ...
    nison_obs, nison_years, nison_rows = nison_inventory(Path(args.nison))
    checks["nison_rows_2016_2024"] = nison_rows
    checks["nison_years"] = sorted(nison_years)
    if nison_obs != NISON_ALLOWED:
        failures.append(
            f"Nison rule family mismatch: observed={len(nison_obs)} expected={len(NISON_ALLOWED)} "
            f"missing={sorted(NISON_ALLOWED - nison_obs)} unknown={sorted(nison_obs - NISON_ALLOWED)}"
        )

    for label, df in (("MarketState", market), ("MTF", mtf), ("Murphy", murphy), ("HistoricalContext", hc), ("HistoricalOutcome", ho)):
        years = set(df.timestamp.dt.year.astype(int))
        checks[label + "_years"] = sorted(years)
        if 2025 in years:
            failures.append(label + ": 2025 present")

    mids = murphy_rule_values(murphy)
    checks["murphy_rule_ids"] = sorted(mids)
    checks["murphy_rule_count"] = len(mids)
    unknown = sorted(mids - MURPHY_ALLOWED)
    blocked = sorted(mids & BLOCKED)
    if unknown:
        failures.append(f"Unknown Murphy rules: {unknown}")
    if blocked:
        failures.append(f"Blocked Murphy rules observed: {blocked}")
    missing = sorted(MURPHY_ALLOWED - mids)
    if missing:
        warnings.append(f"Murphy historical coverage partial; {len(missing)} allowlisted rules remain NOT_EVALUABLE")
    ...
    for key, df in (("market_state_asof_pct", market), ("mtf_asof_pct", mtf), ("historical_context_asof_pct", hc), ("historical_outcome_asof_pct", ho)):
        ...
        if checks[key] <= 0:
            failures.append(key + ": zero coverage")

    sim = Path(args.similarity)
    ret = Path(args.retrieval)
    sim_files = list(sim.rglob("*.json")) if sim.is_dir() else []
    ret_files = list(ret.rglob("*.json")) if ret.is_dir() else []
    checks["similarity_artifact_present"] = bool(sim_files)
    checks["retrieval_artifact_present"] = bool(ret_files)
    if not sim_files:
        failures.append("Similarity V2 JSON artifacts missing")
    if not ret_files:
        failures.append("Retrieval V2 JSON artifacts missing")
    ...
    try:
        ...
        if checks["tiz_missing_status"] != "NOT_EVALUABLE":
            failures.append("TIZ missing-evidence contract failed")
    except Exception as exc:
        failures.append(f"TIZ adapter failed: {exc}")
    ...
    if not Path(args.decision_brain).exists():
        failures.append("Decision Brain V1 missing")
    ...
    try:
        ...
        b.assess(sample, similarity=None)
        checks["brain_executes"] = True
    except Exception as exc:
        failures.append(f"Decision Brain failed: {exc}")
    ...
    checks.update({
        ...
        "similarity_generates_direction": False,
        "retrieval_generates_direction": False,
    })
    report = {
        "status": "PASS" if not failures else "FAIL",
        "gate": "GOVERNED_INTEGRATION_GATE_V4",
