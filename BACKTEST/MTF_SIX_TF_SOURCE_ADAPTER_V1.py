...
def inspect_csv(path: Path, max_rows: int) -> dict[str, Any]:
    ...
    ts_col = normalize_timestamp_column(columns)
    ohlc = normalize_ohlc(columns)

    sampled = pd.read_csv(path, nrows=max_rows, low_memory=False)
    if ts_col is None:
        raise ValueError(f"{path}: no timestamp/time column")
    if len(ohlc) != 4:
        raise ValueError(f"{path}: missing OHLC columns; found={sorted(ohlc)}")

    ts = pd.to_datetime(sampled[ts_col], utc=True, errors="coerce", format="mixed")
    if ts.isna().any():
        raise ValueError(f"{path}: invalid timestamp in sampled rows")
...
def discover(root: Path, max_rows: int) -> tuple[dict[str, list[dict[str, Any]]], list[str]]:
    ...
    for path in sorted(candidates):
        ...
        if tf is None:
            try:
                header = pd.read_csv(path, nrows=0, low_memory=False)
                tf_cols = [c for c in header.columns if str(c).lower() in TIMEFRAME_COLUMNS]
                if tf_cols:
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        "notes": [
            ...
            "A source containing 2025 remains a source artifact; the development pipeline must explicitly exclude 2025 rows.",
        ],
    }
